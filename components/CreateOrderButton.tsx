...
export const CreateOrderButton = ({tokenId}: { tokenId: string }) => {
    ...
    useEffect(() => {
            const fetchOrderForTokenId = async (tokenId: string) => {
                if (hypercertExchange && hypercertExchange.api && tokenId) {
                    const _orders = await hypercertExchange.api.fetchOrders({claimTokenIds: [tokenId]})
    ...
    if (orders && orders.length > 0) {
        return (<Button onClick={cancelOrdersOnClick} colorScheme={"red"}>Cancel sale</Button>)
    }
